...
class CompressingFileThreadWorker(QRunnable):
    ...
    def run(self) -> None:
        ...
        """
        #get from hydrus
        image = hydrus_api_caller.get_full_image(self.file_tile.file_id)
        ...
        # send file to hydrus
        res = hydrus_api_caller.send_to_hydrus(output_image_path)
        if "error" in res:
        ...
        hydrus_api_caller.add_tags_hash(new_hash, self.file_tile.storage_tags)
        self.progress_box.set_progress_text("Duplicating ratings to new file")
        hydrus_api_caller.add_ratings(new_hash, self.file_tile.ratings)
        self.progress_box.set_progress_text("Duplicating notes to new file")
        hydrus_api_caller.add_notes(new_hash, self.file_tile.notes)
        self.progress_box.set_progress_text("Sending old file to hydrus trash")
        hydrus_api_caller.delete_file(self.file_tile.file_id)
